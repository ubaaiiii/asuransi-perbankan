	              	...
	              	<div class="col-md-12 col-sm-12 col-xs-12">
						...
						<table id="datatable-responsive" class="table table-striped table-bordered dt-responsive table-responsive nowrap" cellspacing="0" width="100%">
		                    ...
		                    <tbody>
								...
								<tr>
									<th>
										<button type="button" class="btn btn-default" data-toggle="tooltip" data-placement="top" title="Detail" onclick="klik('<?= $billing->regid ?>')"><i class="fa fa-search"></i> Detail</button>
										<script>
											function klik(key){
									...
									<td>{{ $billing->certno}}</td>
									<td>{{ $billing->regid}}</td>
									<td>{{ $billing->nama}}</td>
									<td>{{ $billing->billdt}}</td>
									<td>{{ number_format($billing->up)}}</td>
									<td>{{ number_format($billing->grossamt)}}</td>
